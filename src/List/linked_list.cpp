...
auto LinkedList::insertFirst(int value) -> void {
    std::unique_ptr<Node> node = std::make_unique<Node>(value);

    if (!tail) tail = node.get();
    node->next = std::move(head);
    head = std::move(node);
}

auto LinkedList::insertLast(int value) -> void {
    std::unique_ptr<Node> node = std::make_unique<Node>(value);
...
auto LinkedList::search(int value) -> Node * {
    Node *tmp = head.get();
    for (; tmp; tmp = tmp->next.get()) 
        if (value == tmp->data) 
            return tmp;
    return nullptr;
}

auto LinkedList::deleteValue(int value) -> void{
    for (Node *tmp = head.get(), *previous = nullptr; tmp; previous = tmp, tmp = tmp->next.get()) {
        if (tmp->data == value) {
            if (previous) {
                previous->next = std::move(tmp->next);
                if (!previous->next) tail = previous;
            } else {
